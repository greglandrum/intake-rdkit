...
class SDFSource(intake.source.base.DataSource):
    ...
    def __init__(self, filename, metadata=None, nPartitions=1, **kwargs):
        super(SDFSource, self).__init__(metadata=metadata)
        self._fname = filename
        if filename.endswith('.gz'):
            self.suppl_factory = lambda fname, **kwargs: Chem.ForwardSDMolSupplier(
                gzip.open(fname), **kwargs)
            if nPartitions != 1:
                raise ValueError(
                    'can only read compressed files with a single partition')
        for k in kwargs:
            if kwargs[k] == 'None':
                kwargs[k] = None
    ...
    def _get_schema(self):
        ...
        return intake.source.base.Schema(datashape=None,
                                         dtype=dt,
                                         shape=(None, len(dt)),
